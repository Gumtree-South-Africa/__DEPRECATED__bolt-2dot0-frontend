...
(function() {
	...
	if (!headerToggleInteraction) {
		$(function() {
			var $header = $(".header"), $signin = $header.find(".signin");
	...
	if (headerToggleInteraction) {
		$(function() {
			var $header = $(".header"), $nav = $header.find(".nav");
	...
	BOLT.UTILS.formatNumber = function(number, places) {
		...
		var decimal = Bolt.DECIMAL, placeholder = Bolt.PLACEHOLDER;
		...
		function getWholePart() {
			var wp = Math.floor(number), bwp = wp.toString().split("").reverse().join("");

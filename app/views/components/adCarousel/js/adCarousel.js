...
(function() {
	$(document).ready(function() {
		...
		var ads = adList.ads, nextUrl = adList.nextAjaxUrl, prevUrl = adList.previousAjaxUrl, $prev = null, $next = null, i = 0, pos = 0, oldNo = 0,

			adSizeLg = 122, adSizeSm = 78,

			imgSizeLg = 100, imgSizeSm = 60,

			panlObj = {
				leftnodesImages: [], midnodesImages: [], rightnodesImages: []
			},

			noofimages = 0, imageWidth = 60, adsRemaining = 8, currentImageIndex = -1,

			panelDirection = null, panelProperties = {}, panelCnt = 3, midWidth = 0,

			firstIE = true, ie8 = false, ie9 = false, badBrowser = false;
		...
		function init() {
			...
			if (!nextUrl) {
				...
				ads.push({
					primaryImgUrl: "#", title: "endAd", viewPageUrl: "#"
				});
		...
		function setNavigation() {
			if (ads.length <= currentImageIndex && ads.length <= noofimages) {
				...
				panlObj.ctrlRight.on("click", function() {
					movePanel(-1);
				});
				...
				panlObj.ctrlLeft.on("click", function() {
					movePanel(1);
				});
		...
		function createPanelObj(imgSpacing, adSize, imgSize) {
			var contentWidth = $(".content").width(), panelWidth, number,

				panelObj = {
					count: 8, imgWidth: 100, midWidth: 1076
				};
		...
		function createAds() {
			var leftUl = $("<ul/>"), midUl = $("<ul/>"), rightUl = $("<ul/>"), diff = Math.abs(noofimages - oldNo);
			...
			if (ads.length > noofimages) {
				for (i = 0; i < noofimages; ++i) {
					...
					panlObj.leftnodesImages.push($("<a/>").attr({"href": "#"}).append($("<div/>").addClass("c-img-container").append($("<img/>").attr("src", "").css({
						width: imageWidth + "px",
					...
					if (ads[pos]) {
						if (ads[pos].title === "endAd") {
							panlObj.midnodesImages.push($("<a/>").attr({"href": ads[pos].viewPageUrl}).append($("<div/>").append(createEndAd())));
						} else {
							panlObj.midnodesImages.push($("<a/>").attr({"href": ads[pos].viewPageUrl}).append($("<div/>").addClass("c-img-container").append($("<img/>").attr("src", ads[pos].primaryImgUrl).css({
								width: imageWidth + "px",
								height: imageWidth + "px"
					...
					if (ads[pos + noofimages]) {
						panlObj.rightnodesImages.push($("<a/>").attr({"href": "#"}).append($("<div/>").addClass("c-img-container").append($("<img/>").attr("src", ads[pos + noofimages].primaryImgUrl).css({
							width: imageWidth + "px",
							height: imageWidth + "px"
						}))));
					}
				...
				for (i = 0; i < ads.length; ++i) {
					if (ads[pos]) {
						if (ads[pos].title === "endAd") {
							panlObj.midnodesImages.push($("<a/>").attr({"href": ads[i].viewPageUrl}).append($("<div/>").append(createEndAd())));
						} else {
							panlObj.midnodesImages.push($("<a/>").attr({"href": ads[pos].viewPageUrl}).append($("<div/>").addClass("c-img-container").append($("<img/>").attr("src", ads[pos].primaryImgUrl).css({
								width: imageWidth + "px",
								height: imageWidth + "px"
		...
		function createSeeAll() {
			var seeAllUrl = window.seeAllUrl, seeAllText = window.seeAllText;

			var $a = $("<a>", {
				"href": seeAllUrl, "target": "top", "class": "sudo-link", "data-o-uri": seeAllUrl
			}).text(seeAllText + " \u003e"), seeAll = $("<div>", {
		...
		function bindEvents() {
			...
			$(".galleryWrapper").on("webkitTransitionEnd transitionend msTransitionEnd oTransitionEnd", panlObj.gallery, function() {
				transitionAds(panelDirection);
			});
		}
		...
		function movePanel(direction) {
			...
			if (direction < 0) { // next (right)
				if (currentImageIndex + noofimages > ads.length && currentImageIndex >= ads.length) {
					return;
				}
				if (ie8 || ie9) {
		...
		function animateAds(newLeft) {
			panlObj.gallery.stop().animate({left: newLeft}, 500, function() {
				transitionAds(panelDirection);
			});
		}
		...
		function transitionAds(panelDirection) {
			...
			var midIndex, leftIndex, rightIndex, vis;
		...
		function createEndAd() {
			var div = $("<div/>").addClass("endAd"), a = $("<span/>"), text = $("<div/>").addClass("postText").text(window.dataPostAdText);
			postLinker(a);

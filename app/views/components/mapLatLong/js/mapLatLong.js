...
BOLT.MapLatLong = (function() {
	...
	function renderMapWithCoords(lat, long) {
		...
		dataLatLong = {
			lat: parseFloat(lat.toFixed(5)), long: parseFloat(long.toFixed(5))
		};
		...
		marker = new google.maps.Marker({
			position: myLatlng, title: "Click to view info!", map: map, draggable: true
		});
		marker.setMap(map);

		// Add circle overlay and bind to marker
		var circle = new google.maps.Circle({
			map: map, radius: 3200,    // 10 miles in metres
			fillColor: '#2FF1F4'
		});
		...
		google.maps.event.addListener(map, "click", function(e) {
			...
			dataLatLong = {
				lat: parseFloat(latLng.lat().toFixed(5)), long: parseFloat(latLng.lng().toFixed(5))
			};

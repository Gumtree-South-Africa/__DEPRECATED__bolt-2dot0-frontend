...
(function(PA) {  // BOLT.POSTAD
	...
	(function() {
		...
		// @todo: change
		var defaultMenuHtml = '<nav id="mmenu-template-list">' + '    <div>' + '        <div class="base-panel-container">' + '            <ul>' + '                <li><span class="mm-arrow base-panel" href="#"><span>TEMPLATE</span></span></li>' + '            </ul>' + '        </div>' + '    </div>' + '    <div class="item-list Panel"></div>' + '</nav>';
				...
				renderUI: function() {
					...
					pubscope.menuObj = $(pubscope.menuContainerSel)
						.mmenu({
							...
							navbar: {
								title: pubscope.config.title, titleLink: "none"
							},
							...
							searchfield: {
								...
								placeholder: pubscope.config.searchPlaceholder || "", // noResults: pubscope.config.noResultsLabel || "",
								showSubPanels: false,
								showTextItems: false
							},
					...
					$(pubscope.menuContainerSel).on('click', 'li.nav-item[data-id] a', function(e) {
						...
						if (isLeaf && isLeaf !== "false") {
							$(document).trigger("MobileLeafReached", {
								id: id, label: text, currentInstance: currentInstance
							});

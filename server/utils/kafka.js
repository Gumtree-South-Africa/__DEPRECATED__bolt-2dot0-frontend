...
var KafkaService = function() {
	var Producer = kafka.Producer, KeyedMessage = kafka.KeyedMessage, Client = kafka.Client, client = new kafka.Client(config.get('kafka.server.host') + ':' + config.get('kafka.server.port'));

	this.producer = new Producer(client), this.topic = config.get('kafka.server.topic');
};
...
KafkaService.prototype.logInfo = function(locale, msg) {
	...
	scope.producer.send(payloads, function(err, data) {
		if (err) {

...
var bapiPromiseGet = function(bapiOptions, bapiHeaderValues, serviceName){
	console.time(`${process.pid} Instrument-BAPI-${serviceName} ${bapiHeaderValues.locale} ${bapiHeaderValues.requestId}`);
	...
	return bapi.doGet(bapiOptions, null).then((output) => {
		...
		if(typeof output === undefined || output.statusCode) {
			...
			console.timeEnd(`${process.pid} Instrument-BAPI-${serviceName} ${bapiHeaderValues.locale} ${bapiHeaderValues.requestId}`);
			return output;
		}
	});
};
